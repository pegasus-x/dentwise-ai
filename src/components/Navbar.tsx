...
function Navbar() {
  ...
  // 🔥 Smart switch
  const navLinks = pathname === "/" ? landingLinks : appLinks;
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled
          ? "top-4 mx-4 md:mx-12 rounded-[2rem] border border-white/10 bg-background/40 backdrop-blur-2xl shadow-lg"
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex justify-between items-center">
        ...
        {/* LOGO */}
        <Link href={pathname === "/" ? "/" : "/dashboard"} className="flex items-center gap-2">
          <Image src="/logo.png" alt="DentWise" width={36} height={36} />
          <span className="font-bold text-xl hidden sm:inline">DentWise</span>
        </Link>
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium text-muted-foreground hover:text-foreground transition"
              >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="md:hidden mx-4 mt-2 p-4 rounded-xl bg-background border"
          >
            {navLinks.map((link) => {
              const Icon = (link as any).icon;
              ...
              return (
                <Link
                  ...
                  href={link.href}
                  onClick={() => handleNavClick(link.href)}
                  className="flex items-center gap-3 py-3 px-2 text-lg"
                >
